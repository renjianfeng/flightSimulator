   ...
   // this.rect1.linkOffsetY = -50;
     ...
     /*    var hit = scene.pickWithRay(ray);
      ...
      /*   this.probe = new BABYLON.ReflectionProbe("main", 64, this.scene);
        ...
        for(var i=0;i<=160;i++){
            this.frees[i]= BABYLON.MeshBuilder.CreateSphere("frees", {diameterX:  0.5, diameterY: 0.5, diameterZ: 7}, this.scene);
       ...
       /*  console.log("this.scene")
        console.log(this.scene) */
       /*  var sky=this.scene.createDefaultSkybox(AssetsManager.ins.resourceObject["cubeTextures"]["gameScene"]["skybox"], true, 10000);
        sky.applyFog=false;
        console.log("sky")
        console.log(sky) */
                    ...
                    var jl2=this.getDistance(
                     ...
                     if(jl2<=50){
                      ...
                      //  this.booms[i].scaling=new BABYLON.Vector3(1,1,1);
                        setTimeout(()=>{
                            this.booms[i].boom.to({ x:10 ,y:10,z:10}, 200);
                            this.booms[i].boom.start();
                            this.booms[i].boom.onComplete(()=>{
                                this.booms[i].scaling.x=0.1;
                                this.booms[i].scaling.y=0.1;
                                this.booms[i].scaling.z=0.1;
                             })
                        },100)
