...
export class ExGameSceneCon extends GameScenes{
    ...
    protected resetGame(){
        ...
        for(var i=0;i<=60;i++){
            this.frees[i]= BABYLON.MeshBuilder.CreateBox("frees", {height:  0.5, width: 0.5, depth: 10}, this.scene);
    ...
    private freeUpdate() {
        ...
        this.frees.forEach((free)=>{
            if(free.lifeState==true){
                var forword=new BABYLON.Vector3(free.forward.x*15,free.forward.y*15,free.forward.z*15)
