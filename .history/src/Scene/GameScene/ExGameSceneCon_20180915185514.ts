...
export class ExGameSceneCon extends GameScenes{
    ...
    protected resetGame(){
      ...
      /*   this.probe = new BABYLON.ReflectionProbe("main", 64, this.scene);
        ...
        for(var i=0;i<=60;i++){
            this.frees[i]= BABYLON.MeshBuilder.CreateBox("frees", {height:  0.2, width: 0.2, depth: 1}, this.scene);
        ...
        for(var i=0;i<=60;i++){
            this.booms[i]= BABYLON.MeshBuilder.CreateSphere("boom", {diameter: 1}, this.scene);
    ...
    private freeUpdate() {
        ...
        this.frees.forEach((free,i)=>{
            if(free.lifeState==true){
                ...
                if (free.intersectsMesh(this.scene.getMeshByName("default"), false)) {
