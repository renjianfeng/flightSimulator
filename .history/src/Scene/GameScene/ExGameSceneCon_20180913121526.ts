...
export class ExGameSceneCon extends GameScenes{
    ...
    private setMeshVal(mesh,state,val){
        ...
        var _sd=[];
        _sd["up-go"]=this.scene.beginWeightedAnimation(mesh, 0, 0.7916666865348816*val,0, false);
        _sd["up-back"]=this.scene.beginWeightedAnimation(mesh, 0.7916666865348816, 1.625*val,0, false);
        _sd["down-go"]=this.scene.beginWeightedAnimation(mesh, 1.625, 2.4583332538604736*val,0, false);
        _sd["down-back"]=this.scene.beginWeightedAnimation(mesh, 2.4583332538604736, 3.2916667461395264*val,0, false);
       ...
       if(state=="up-go"){
           console.log("4455")
           _sd["up-go"].weight = 1
           _sd["up-back"].weight = 0
           _sd["down-go"].weight = 0
           _sd["down-go"].weight = 0
          /*  _sd.forEach((sd,i)=>{
               console.log("sd")
               console.log(sd)
       ...
       if(state=="down-go"){
        _sd["up-go"].weight = 0
        _sd["up-back"].weight = 0
        _sd["down-go"].weight = 1
        _sd["down-back"].weight = 0
