...
export class ExGameSceneCon extends GameScenes{
        ...
        var eleImage = document.getElementById('renderCanvas');
            if (eleImage) {
                ...
                var rotate3D =  (event)=> {
                    ...
                    this.display.cameraBox.rotation.x=-moveY*0.001;
                  ...
                  /*   if(_s==moveX){
                        if(this.flayState["右翼"]!="back"){
                            this.setMeshVal(this.scene.getMeshByName("右翼"),"back",1)
                        }
                        console.log("小")
                        if(this.flayState["左翼"]!="back"){
                            this.setMeshVal(this.scene.getMeshByName("左翼"),"back",1)
                        }
                    } */
                    _s=moveX;
    ...
    private setMeshVal(mesh,state,val){
        ...
        var _sd=[];
        _sd["up-go"+mesh.name]=this.scene.beginWeightedAnimation(mesh, 0, 0.7916666865348816*val,0.2, false);
        _sd["up-back"+mesh.name]=this.scene.beginWeightedAnimation(mesh, 0.7916666865348816, 1.625*val,0.2, false);
        _sd["down-go"+mesh.name]=this.scene.beginWeightedAnimation(mesh, 1.625, 2.4583332538604736*val,1, false);
        _sd["down-back"+mesh.name]=this.scene.beginWeightedAnimation(mesh, 2.4583332538604736, 3.2916667461395264*val,0.2, false);
        _sd["back"+mesh.name]=this.scene.beginWeightedAnimation(mesh, 3.1916667461395264*val, 3.2916667461395264*val,1, false);
