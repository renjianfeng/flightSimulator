...
export class HotDecoyFlareCon{
    ...
    public init(display){
        ...
        //创建子弹列表
        for(var i=0;i<=160;i++){
            this.bullets[i]= BABYLON.MeshBuilder.CreateSphere("frees", {diameterX:  0.5, diameterY: 0.5, diameterZ: 0.5}, this.scene);
    ...
    public update(times) {
        ...
        // this.k+=0.1
         this.free.rotation.x+=0.1

         this.bullets.forEach((free,i)=>{
             if(free.lifeState==true){
                 var forword=new BABYLON.Vector3(free.forward.x*this.times*this.fireSpeed*k,free.forward.y*this.times*this.fireSpeed*k,free.forward.z*this.times*this.fireSpeed)
