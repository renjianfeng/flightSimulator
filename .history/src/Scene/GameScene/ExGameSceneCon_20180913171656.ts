        ...
        setInterval(()=>{
            if(this.j<=this.frees.length-1){
              ...
              /*   for(let i=this.j;i<this.frees.length;i++){
                ...
                this.frees[this.j].position=new BABYLON.Vector3(this.display.cameraBox.position.x,this.display.cameraBox.position.y,this.display.cameraBox.position.z) ;
    ...
    private freeUpdate() {
        ...
        this.frees.forEach((free)=>{
            if(free.lifeState==true){
                var forword=new BABYLON.Vector3(free.forward.x*50,free.forward.y*50,free.forward.z*50)
