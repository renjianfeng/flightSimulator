...
export class ExGameSceneCon extends GameScenes{
    ...
    protected resetGame(){
       ...
       setInterval(()=>{
          l_state=!l_state
          if(l_state){
            this.scene.getMaterialByName("Pilot Head").emissiveColor=new BABYLON.Color3(0.5,0.5,0.5);
          }else{
            this.scene.getMaterialByName("Pilot Head").emissiveColor=new BABYLON.Color3(0.6,0.6,0.6);
          }
       },100)
       ...
       setInterval(()=>{
            ...
            setTimeout(()=>{
                this.scene.getMeshByName("灯").isVisible=false;
            },50)
       },1000)
