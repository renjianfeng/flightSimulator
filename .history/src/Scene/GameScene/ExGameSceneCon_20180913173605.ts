...
export class ExGameSceneCon extends GameScenes{
    ...
    protected resetGame(){
        ...
        for(var i=0;i<=30;i++){
            this.frees[i]= BABYLON.MeshBuilder.CreateBox("frees", {height:  0.5, width: 0.5, depth: 2}, this.scene);
    ...
    private freeUpdate() {
        ...
        this.frees.forEach((free)=>{
            if(free.lifeState==true){
                var forword=new BABYLON.Vector3(free.forward.x*40,free.forward.y*40,free.forward.z*40)
